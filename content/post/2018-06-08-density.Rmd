...
knitr::include_graphics('/img/posts/2017-06-08-density/density-bin.png', error = FALSE)
```
...
knitr::include_graphics('/img/posts/2017-06-08-density/mvt.png', error = FALSE)
```
...
knitr::include_graphics('/img/posts/2017-06-08-density/subparts.png', error = FALSE)
```
...
$$
\begin{align}
  \hat{f_0}(x) =& \frac{1}{nh}(v_0 + \cdots + v_{m - 1})\\
  \hat{f_1}(x) =& \frac{1}{nh}\left(v_{-1} + v_0 + \cdots v_{m - 2}\right) \\
  \hat{f_2}(x) =& \frac{1}{nh}\left(v_{-2} + v_{-1} + v_0 + \cdots v_{m - 3}\right) \\
  \cdots&\\
  \hat{f_i}(x) =& \frac{1}{nh}\left(\sum_{j=1}^{i} v_{-j} + v_0 + \sum_{k = 1}^{m - i + 1}v_{k}\right)\\
  \cdots&\\
  \hat{f}_{m - 1}(x) =& \frac{1}{nh}\left(\sum_{j=1}^{m - 1} v_{-j} + 0\right)
\end{align}
$$
